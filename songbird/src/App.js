...
class App extends React.Component {
  ...
  handleVariants = (event) => {
    if (event.target.closest('.variant') === null) { //check when css is ready
      return;
    }
      ...
      if (this.state.isGuessedRight === false) {
        this.setState({
          ...
          birds: {
            ...
            image: birdsData[this.state.activeTabNum][this.state.randomNum].image
          }
        })
    ...
    const handleFalseVariant = () => {
      if (this.state.addScore > 0 && this.state.pressedVariants.has(index) === false) {
        this.setState({
    ...
    +index === this.state.randomNum ? handleTrueVariant() : handleFalseVariant();
  }
    ...
    <div className="App">
      ...
      <div className="choose_wrapper">
        ...
        <Description
          ...
          image={birdsData[this.state.activeTabNum][this.state.chosenVariant].image} 
          name={birdsData[this.state.activeTabNum][this.state.chosenVariant].name}
          latin={birdsData[this.state.activeTabNum][this.state.chosenVariant].species}
          audio={birdsData[this.state.activeTabNum][this.state.chosenVariant].audio}
          description={birdsData[this.state.activeTabNum][this.state.chosenVariant].description}
        />
      </div>      
      <ButtonNext 
        name='Next Level' 
      ...
      <div className={this.state.end}> 
        ...
        <h2 className={this.state.score === 30 ? '' : initialState.endImage}>Daamn. You are an absolute winner!</h2>
        <img style={{width: '80%'}} src="https://media.giphy.com/media/44gu1V41ejJni/giphy.gif" className={this.state.score === 30 ? '' : initialState.endImage} alt="winner"/>
        <ButtonNext 
          name='Try again!' 
          active={this.state.isButtonNextActive}
          onClick={() => this.buttonNextHandler()}
